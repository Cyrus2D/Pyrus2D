...
class dlog:
    ...
    def add_circle(level: Level = Level.LEVEL_ANY,
                   ...
                   color: Color = Color(string='red'), ):
        if cx is not None:
            dlog._commands += f"{dlog._time.cycle()},{dlog._time.stopped_cycle()} {level.value} {'C' if fill else 'c'} {cx} {cy} {r} {color}\n"
    ...
    def add_point(level: Level.LEVEL_ANY,
                  ...
                  color: Color = Color(string='red')):
        if x is not None:
            dlog._commands += f"{dlog._time.cycle()},{dlog._time.stopped_cycle()} {level.value} p {x} {y} {color}"
    ...
    def add_message(level: Level.LEVEL_ANY,
                    ...
                    y,
                    msg):
        dlog._commands += f"{dlog._time.cycle()},{dlog._time.stopped_cycle()} {level.value} m {round(x, 4)} {round(y, 4)} {msg}\n"
