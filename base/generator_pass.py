...
class BhvPassGen(BhvKickGen):
    ...
    def update_receivers(self, wm: 'WorldModel'):
        ...
        for tm in wm.teammates():
            ...
            if tm.is_tackling():
                ...
                continue
            if tm._pos.x() > wm.offside_line_x():
                log.sw_log().pass_().add_text(f'-----<<< TM is in offside {tm.pos().x()} > {wm.offside_line_x()}')
                continue
            if tm.goalie() and tm._pos.x() < sp.our_penalty_area_line_x() + 15:
        ...
        self.receivers = sorted(self.receivers, key=lambda p: p._pos.x(), reverse=True)
    ...
    def generate_direct_pass(self, wm: 'WorldModel', receiver: 'PlayerObject'):
        ...
        if receiver._pos.x() > sp.pitch_half_length() - 1.5 \
                or receiver._pos.x() < -sp.pitch_half_length() + 5.0 \
                or receiver._pos.abs_y() > sp.pitch_half_width() - 1.5:
            if debug_pass:
                log.sw_log().pass_().add_text( '#DPass to {} {}, out of field'.format(receiver.unum(), receiver.pos()))
            return
        # TODO sp.ourTeamGoalPos()
        if receiver._pos.x() < wm.ball()._pos.x() + 1.0 \
                and receiver._pos.dist2(Vector2D(-52.5, 0)) < pow(18.0, 2):
            if debug_pass:
        ...
        receive_point = ptype.inertiaFinalPoint(receiver._pos, receiver._vel)
        ball_move_dist = wm.ball()._pos.dist(receive_point)
        ...
        ball_move_angle = (receive_point - wm.ball()._pos).th()
        ...
        log.sw_log().pass_().add_text( '#DPass to {} {}'.format(receiver.unum(), receiver._pos))
    ...
    def generate_lead_pass(self, wm: 'WorldModel', receiver):
        ...
        if receiver._pos.dist(wm.ball()._pos) > max_player_distance:
            if debug_pass:
        ...
        angle_from_ball = (receiver._pos - wm.ball()._pos).th()
        for d in range(1, dist_divs + 1):
            ...
            for a in range(abgle_divs + 1):
                ...
                ball_move_line = Line2D(wm.ball()._pos, receive_point)
                player_line_dist = ball_move_line.dist(receiver._pos)
                ...
                if receive_point.x() < wm.ball()._pos.x() \
    ...
    def predict_receiver_reach_step(self, receiver, pos: Vector2D, use_penalty, pass_type):
        ...
        n_turn = 1 if receiver.body_count() > 0 else Tools.predict_player_turn_cycle(ptype, receiver._body,
                                                                                     receiver._vel_r, target_dist, (
        ...
        if pass_type == 'L':
            ...
            dash_angle = (pos - receiver._pos).th()

            if dash_angle.abs() > 90.0 or receiver.body_count() > 1 or (dash_angle - receiver._body).abs() > 30.0:
                n_turn += 1
    ...
    def create_pass(self, wm: 'WorldModel', receiver, receive_point: Vector2D,
        ...
        for step in range(min_step, max_step + 1):
            ...
            o_step, o_unum, o_intercepted_pos = self.predict_opponents_reach_step(wm, wm.ball()._pos,
    ...
    def predict_opponent_reach_step(self, wm: 'WorldModel', opp: 'PlayerObject', first_ball_pos: Vector2D, first_ball_vel: Vector2D,
        ...
        min_cycle = Tools.estimate_min_reach_cycle(opponent._pos, ptype.real_speed_max(), first_ball_pos,
                                                   ball_move_angle)
        ...
        for cycle in range(max(1, min_cycle), max_cycle + 1):
            ...
            inertia_pos = ptype.inertia_point(opponent._pos, opponent._vel, cycle)
            ...
            if opponent.body_count() > 1:
                n_turn = Tools.predict_player_turn_cycle(ptype, opponent._body, opponent._vel_r, target_dist,
